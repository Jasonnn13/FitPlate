...
import { ChevronLeft, Plus, Clock, Camera, Home, Book, User, Check } from "lucide-react"
...
export default function AddConsumedMenuPage({
  ...
  // Pre-fill form when a recipe is selected
  useEffect(() => {
    if (selectedRecipe) {
      setMealName(selectedRecipe.name)
      setPortion(`${selectedRecipe.servings} serving(s)`)
      setIsRecipeSelected(true)

      // If the recipe has ingredients, use them
      if (selectedRecipe.ingredients && selectedRecipe.ingredients.length > 0) {
        onUpdateIngredients(selectedRecipe.ingredients)
      }
    }
  }, [selectedRecipe, onUpdateIngredients])
  ...
  return (
    <div className="max-w-sm mx-auto bg-[#ffffff] min-h-screen">
      ...
      <div className="px-4 pb-20 space-y-6">
        ...
        {isRecipeSelected ? (
          <div className="bg-[#e8f4fd] border border-[#007aff] rounded-2xl p-4">
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 bg-[#007aff] rounded-full flex items-center justify-center flex-shrink-0">
                <Check className="w-6 h-6 text-white" />
              </div>
              <div className="flex-1">
                <h3 className="text-lg font-bold text-[#000000]">Recipe Selected</h3>
                <p className="text-[#787880] text-sm">{selectedRecipe?.name}</p>
              </div>
              <Button variant="outline" onClick={onNavigateToPickRecipe} className="text-[#007aff] border-[#007aff]">
                Change
              </Button>
            </div>
          </div>
