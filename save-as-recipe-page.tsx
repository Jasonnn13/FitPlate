...
export default function SaveAsRecipePage({
  ...
  const { addConsumedMeal, loading } = useNutrition()
  ...
  const handleSaveMeal = async () => {
    ...
    try {
      const result = await addConsumedMeal({
        ...
        ingredients: mealData.selectedIngredients.map((ing) => ({
          id: ing.id,
          amount: ing.amount,
          unit: ing.unit,
        })),
      ...
      if (result) {
        setSavedCalories(result.calculatedCalories)
        // Auto-navigate to home after a short delay to show success
        setTimeout(() => {
          onNavigateToHome()
        }, 1500)
      }
  ...
  const handleSaveAsRecipe = async () => {
    // First save the meal, then navigate to recipe creation
    ...
    <div className="max-w-sm mx-auto bg-[#ffffff] min-h-screen">
      ...
      <div className="px-4 pb-20 space-y-6">
        ...
        <div className="text-center py-6">
          ...
          <h2 className="text-2xl font-bold text-[#000000] mb-2">Ready to Save!</h2>
          <p className="text-[#787880] text-base">Your meal is ready to be added to your daily intake.</p>
          {savedCalories && (
            <div className="mt-3 p-3 bg-green-50 border border-green-200 rounded-lg">
              <p className="text-green-700 font-medium">
                ✅ Meal saved! Added {savedCalories} calories to your daily total.
              </p>
            </div>
          )}
        </div>
        ...
        <div className="bg-[#e8f4fd] border border-[#007aff] rounded-2xl p-6">
          ...
          <Button
            ...
            className="w-full bg-[#007aff] hover:bg-[#0056b3] text-white rounded-2xl py-3 text-base font-medium disabled:opacity-50"
          >
            <Plus className="w-5 h-5 mr-2" />
            {isSaving ? "Saving..." : "Create Recipe from This Meal"}
          </Button>
        </div>
        ...
        <div className="space-y-3">
          <Button
            ...
            disabled={isSaving || loading || savedCalories !== null}
            className="w-full bg-[#2c2c2c] hover:bg-[#1d1b20] text-white rounded-2xl py-4 text-base font-medium disabled:opacity-50"
          >
            {isSaving ? "Saving..." : savedCalories ? "Saved! Going to Home..." : "Save Meal & Go to Home"}
          </Button>
